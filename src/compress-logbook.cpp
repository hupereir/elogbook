...
int main (int argc, char *argv[])
{
    ...
    if( argc < 2 )
    {
        Debug::Throw(0) << "usage: compress-logbook <input file>" << Qt::endl;
        return 0;
    }
    ...
    Debug::Throw(0) << "compress-logbook - reading from: " << input << Qt::endl;
    Logbook logbook;
    logbook.setFile( input.expanded() );
    if( !logbook.read() )
    {
        Debug::Throw(0) << "compress-logbook - error reading logbook" << Qt::endl;
        return 0;
    }

    // debug
    Debug::Throw(0) << "compress-logbook - number of files: " << logbook.children().size() << Qt::endl;
    Debug::Throw(0) << "compress-logbook - number of entries: " << logbook.entries().size() << Qt::endl;

    // perform copy
    Debug::Throw(0) << "compress-logbook - compressing" << Qt::endl;
    logbook.setUseCompression( true );
    logbook.setModifiedRecursive( true );

    // copy logbook to ouput
    if( !logbook.write() )
    { Debug::Throw(0) << "compress-logbook - error writing to file " << input << Qt::endl; }
...
void interrupt( int sig )
{
    Debug::Throw() << "compress-logbook::interrupt - Recieved signal " << sig << Qt::endl;
    exit(0);
}
