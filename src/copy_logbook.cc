...
int main (int argc, char *argv[])
{
  try {
    ...
    signal(SIGINT,  Interupt_CB);
    signal(SIGTERM, Interupt_CB);
    ...
    if( fullname.exist() ) {
      ...
      if( !path.isDirectory() ) {
        cout << "copy_logbook - selected path is not vallid. <Save Logbook> canceled." << endl;
        return 0;
      }
...
void Interupt_CB( int sig )
{ 
  Debug::Throw() << "Interupt_CB - Recieved signal " << sig << endl;
  exit(0);
}
