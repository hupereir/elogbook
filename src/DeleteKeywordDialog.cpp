...
DeleteKeywordDialog::DeleteKeywordDialog( QWidget* parent, const std::vector<Keyword>& keywords, const bool& has_entries ):
CustomDialog( parent )
{
    ...
    if( keywords.size() == 1 ) what << "Delete keyword " << keywords.front() << " ?";
        ...
        for( std::vector<Keyword>::const_iterator iter = keywords.begin(); iter != keywords.end(); ++iter )
        {
            what << *iter << " ";
            index++;
